...
to passengers-enter-the-train ; applied to a train
  ...
  ask stations with [ station-num = going-to-station-aux ] [
    ifelse (num-passengers-aux + passengers-waiting <= max-capacity) [
      set entering passengers-waiting
      set num-passengers-aux num-passengers-aux + entering
    ] [
      set entering max-capacity - num-passengers-aux
      set num-passengers-aux max-capacity
    ]
    set passengers-waiting passengers-waiting - entering
    set ticks-from-last-train 0
    update-passengers-waiting
  ]
  set num-passengers num-passengers-aux
  print (word "STATION " going-to-station ": " entering " passengers enter the train")
end
...
to calculate-max-passengers
  let train-passengers 0
  ask trains [ set train-passengers num-passengers]
  set max-passengers max(list max-passengers train-passengers)
end
...
to update-passengers-waiting ; applied to a station
  if (station-num != 4) [
    ...
    if minutes-from-last-train =  0.0 [ set passengers-waiting calcRand 100 200 ]
    if minutes-from-last-train =  5.0 [ set passengers-waiting calcRand 150 300 ]
    if minutes-from-last-train = 10.0 [ set passengers-waiting calcRand  50 300 ]
    if minutes-from-last-train = 15.0 [ set passengers-waiting calcRand 200 400 ]
    set label passengers-waiting
  ]
end
